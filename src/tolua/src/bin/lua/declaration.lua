...
function classDeclaration:checkname ()
 ...
 local b,e,d = strfind(self.type,"list<(.+)>")
 if b then
  print('FOUND LIST<T> "' ..self.type .. '" ' .. d)
  self.list_type = d
  self.type = 'list'
 end

 local b,e,d = strfind(self.type,"vector<(.+)>")
 if b then
  print('FOUND VECTOR<T> "' ..self.type .. '" ' .. d)
  self.list_type = d
  self.type = 'vector'
 end
...
function classDeclaration:decltype ()
 if self.type == 'list' then return 'QList<'..self.list_type..'>' end
 if self.type == 'vector' then return 'QVector<'..self.list_type..'>' end
...
function Declaration (s,kind)
 ...
 if kind == 'var' then
  ...
  print('function '..tostring(v)..' return type= ' .. tostring(tp))
  return _Declaration{
   name = v,
   type = tp,
   mod = md,
